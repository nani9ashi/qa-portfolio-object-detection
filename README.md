# QAポートフォリオ③ 評価・判断編 — 既存物体検出モデルの導入可否評価
...
> **なぜこの題材か** — 引越トラックの助手席で、住宅街の狭い道を進む運転席の死角と、歩行者・自転車のヒヤリとする場面を日常的に見てきた。安全補助システムにとって「見逃し」がどれほど重い失敗かは、体感として知っている。既製の検出モデルをこの用途に導入してよいか——その判断を QA の立場で引き受けたのが本作である。
...
なお、本プロジェクトは一人称のシミュレーションであり、PdM・Dev の発言や承認も自分が役割を演じて GitHub Issue 上に記録したもの（実在の第三者ではない）。それでも役割を分けているのは、導入可否の判断がもつ構造——**誰が基準を承認し、誰が測定し、誰が最終決定するか**——を実務と同じ形で残すためである。

## スコープと限界・次フェーズ

### スコープ

公開データセット（COCO）による事前評価。評価対象は YOLOv8n・入力解像度 640・推論専用に限定。
...
見逃しの底上げは信頼度閾値の調整だけでは不十分（下の PR 曲線のとおり、再現率が頭打ちになる）。推論時間に大きな余裕があるため、より大きいモデル（YOLOv8m/l 等）への変更を AC-5 込みで再評価するのが有望。
